...
def match_grasp_view_and_label(end_points):
    ...
    top_view_inds = end_points['grasp_top_view_inds'].to('cuda:0')  # (B, Ns)
    template_views_rot = end_points['batch_grasp_view_rot'].to('cuda:0')  # (B, Ns, V, 3, 3)
    grasp_scores = end_points['batch_grasp_score'].to('cuda:0')  # (B, Ns, V, A, D)
    grasp_widths = end_points['batch_grasp_width'].to('cuda:0')  # (B, Ns, V, A, D, 3)
